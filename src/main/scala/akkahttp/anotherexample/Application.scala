  ...
  def main(args: Array[String]): Unit = {
    bind().onComplete {
      case Success(binding) =>
        ...
        println(s"${config.getString("app.appName")} is running at http://${localAddress.getHostName}:${localAddress.getPort}")
      case Failure(exception) =>
        println(s"${config.getString("app.appName")} failed to start, the following exception was thrown:\n\n$exception")
        system.terminate()
    }
  }
